...
#define CALC(x,y,z,a,b) (((x)*6) + ((y)*4) + ((z)*2) + ((a)*3) * ((b)+1))
...
{
    printf("\nTEST FILE: %s\n\n", __FILE__);
    printf("This test creates a 5 Dimension LOCal INTEGER ARRAY and displays its details.\n\n");
    printf("EXPECTED RESULTS:\n");
    printf("The address of the new scope to be printed, followed by\n");
    printf("the details of a 5D integer array, named 'testArray', the values\n");
...
/* Function to create a single new scope. */
 void test_1() {
    ...
    beginScope();
    variable = LOCAL_ARRAY_INTEGER5("testArray", maxXDim, maxYDim, maxZDim, maxADim, maxBDim);
    ...
    for (x = 0; x <= maxXDim; x++) {
        for (y = 0; y <= maxYDim; y++) {
            for (z = 0; z <= maxZDim; z++) {
                for (a = 0; a <= maxADim; a++) {
                    for (b = 0; b <= maxBDim; b++) {
                        /* printf("testArray[%d][%d][%d][%d][%d] = %d\n", x, y, z, a,b, CALC(x,y,z,a,b)); */
                        SET_INTEGER_ELEMENT5("testArray", x, y, z, a, b, CALC(x,y,z,a,b));
                    }
                }
            }
        }
    ...
    for (x = 0; x <= maxXDim; x++) {
        for (y = 0; y <= maxYDim; y++) {
            for (z = 0; z <= maxZDim; z++) {
                for (a = 0; a <= maxADim; a++) {
                    for (b = 0; b <= maxBDim; b++) {
                        printf("%04d. Variable->Value[%d][%d][%d][%d][%d] : %d (expected %d)\n", countAll++, x, y, z, a, b, GET_INTEGER_ELEMENT5("testArray", x, y, z, a, b), CALC(x,y,z,a,b));
                    }
                    printf("\n");
                }
                printf("\n");
