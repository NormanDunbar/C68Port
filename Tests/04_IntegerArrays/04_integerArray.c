...
#define CALC(x,y,z,a) (((x)*6) + ((y)*4) + ((z)*2) + (a))
...
{
    printf("\nTEST FILE: %s\n\n", __FILE__);
    printf("This test creates a 4 Dimension LOCal INTEGER ARRAY and displays its details.\n\n");
    printf("EXPECTED RESULTS:\n");
    printf("The address of the new scope to be printed, followed by\n");
    printf("the details of a 4D integer array, named 'testArray', the values\n");
...
/* Function to create a single new scope. */
 void test_1() {
    ...
    beginScope();
    variable = LOCAL_ARRAY_INTEGER4("testArray", maxXDim, maxYDim, maxZDim, maxADim);
    ...
    for (x = 0; x <= maxXDim; x++) {
        for (y = 0; y <= maxYDim; y++) {
            for (z = 0; z <= maxZDim; z++) {
                for (a = 0; a <= maxADim; a++) {
                    /* printf("testArray[%d][%d][%d][%d] = %d\n", x, y, z, a, CALC(x,y,z,a)); */
                    SET_INTEGER_ELEMENT4("testArray", x, y, z, a, CALC(x,y,z,a));
                }
            }
        }
    }
 ...
 void dumpVariable(SBLOCAL variable) {
    ...
    for (x = 0; x <= maxXDim; x++) {
        for (y = 0; y <= maxYDim; y++) {
            for (z = 0; z <= maxZDim; z++) {
                for (a = 0; a <= maxADim; a++) {
                    printf("Variable->Value[%d][%d][%d][%d] : %d (expected %d)\n", x, y, z, a, GET_INTEGER_ELEMENT4("testArray", x, y, z, a), CALC(x,y,z,a));
                }
                printf("\n");
            }
            printf("\n");
